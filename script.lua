...
local FileName = "MeloskaHub_v1.json" 
local DefaultConfig = {
    Positions = {
...
local Theme = {
    ...
    SurfaceHighlight= Color3.fromRGB(18, 26, 52),
    Accent1         = Color3.fromRGB(60, 140, 255),
    Accent2         = Color3.fromRGB(30, 90, 200),
    TextPrimary     = Color3.fromRGB(240, 240, 240),
    TextSecondary   = Color3.fromRGB(130, 155, 190),
...
local function ShowNotification(title, text)
    local existing = PlayerGui:FindFirstChild("MystNotif")
    if existing then existing:Destroy() end

    local sg = Instance.new("ScreenGui", PlayerGui)
    sg.Name = "MystNotif"; sg.ResetOnSpawn = false
...
local function setInfiniteJump(enabled)
    ...
    if Connections.infiniteJumpFallConn then Connections.infiniteJumpFallConn:Disconnect(); Connections.infiniteJumpFallConn = nil end
    ...
    Connections.infiniteJumpFallConn = RunService.Heartbeat:Connect(function()
        local char = LocalPlayer.Character
...
CreateToggleSwitch(rJoinerRow, Config.ShowJobJoiner, function(ns, set)
    ...
    local gui = PlayerGui:FindFirstChild("MystJobJoiner")
...
if not IS_MOBILE then
    UserInputService.InputBegan:Connect(function(input, gp)
        ...
        if Config.JobJoinerKey and input.KeyCode == Enum.KeyCode[Config.JobJoinerKey] then
            local joinerGui = PlayerGui:FindFirstChild("MystJobJoiner")
            if joinerGui then
...
task.spawn(function()
    ...
    if privateBuild then
        ...
        author.Text = "discord.gg/MeloskaHub"
    end
...
task.spawn(function()
    ...
    local GOLD   = Color3.fromRGB(60, 140, 255)
    local DARK   = Color3.fromRGB(5, 5, 10)
    local SURF   = Color3.fromRGB(10, 14, 28)
    local HIGH   = Color3.fromRGB(18, 26, 52)
    local ACCENT = Color3.fromRGB(30, 90, 200)
    local TEXT   = Color3.fromRGB(240, 240, 240)
    local GRAY   = Color3.fromRGB(130, 155, 190)
